...
pub(crate) fn resolve_tool_path(work_dir: &Option<PathBuf>, raw: &str) -> PathBuf {
    let p = Path::new(raw);
...
mod tests {
    ...
    fn test_resolve_absolute_unchanged() {
        let p = resolve_tool_path(&Some(PathBuf::from("/work")), "/abs/file.txt");
    ...
    fn test_resolve_relative_joins_work_dir() {
        let p = resolve_tool_path(&Some(PathBuf::from("/work")), "sub/file.txt");
    ...
    fn test_resolve_relative_work_dir() {
        let p = resolve_tool_path(&Some(PathBuf::from("sub")), "file.txt");
