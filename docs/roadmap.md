...
> 状态：以下方向已全部立项并交付（024/025/027/028/029），本节为历史规划记录
> 依据：各任务审查遗留项 + 架构预留 + 已声明边界

以下为已交付方向的规划记录，每条含：**动机**（引用遗留来源）+ **大致范围** + **新依赖/风险**。实施顺序：024 持久化 lane head → 025 ACP SSE/HTTP（PM 定序，插入到 022/023 自定义模型+TUI 之前）→ 027 schemars → 028 跨进程锁 → 029 Agent 插件（PM 定序「插件最后」，前两项优先级由 Architect 排定，理由见 TASK_BOARD 十期启动记录）。`axum`/`tokio-stream` 已由 PM 签核 feature-gated（`acp-sse` 非 default）。

## 1. 持久化 lane head / 活动分支元数据 ✅ 已交付 024
...
- **状态**：✅ 已交付（024，docs/tasks/024-lane-head-persistence.md）。

## 2. ACP SSE/HTTP 远程多 client ✅ 已交付 025
...
- **状态**：✅ 已交付（025，docs/tasks/025-acp-sse-http.md）。

## 3. schemars 强类型工具参数 ✅ 已交付 027
...
- **状态**：✅ 已交付（027，docs/tasks/027-schemars-tool-params.md）。feature `schema` default 开启；内置工具参数 derive 化；零破坏 `Tool` trait（Value→RootSchema 往返替代双事实源）。
...
- **状态**：✅ 已交付（028，docs/tasks/028-cross-process-lock.md）。`fs2` 内核态锁（崩溃自释放化解遗留锁）；`FileMutationQueue`/`JsonlSessionStorage` 可选叠加，零破坏默认行为。
