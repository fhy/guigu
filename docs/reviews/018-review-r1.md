...
- cargo test --all-targets: ✓（391 passed，0 failed）
- cargo fmt --check: ✓

## 代码审查
### 结论
1. `src/server/lane.rs` 从 420 行缩减至 249 行，`src/server/lane_recovery.rs` 为 192 行，均符合单文件 400 行限制。
2. 对 `eee2de9` 的差异检查显示，恢复入口、回滚 helper、head/transcript helper 均为从 `lane.rs` 原样移动到 `lane_recovery.rs`；`mod.rs` 仅新增模块声明，未发现控制流或公开方法签名变化。
